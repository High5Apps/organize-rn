...
const fakeDirectoryData = '<directory>';
...
class FileSystemMock {
   ...
   */
  public isDir = jest.fn(
    async (path: string) => this.filesystem.get(path) === fakeDirectoryData,
  );

  /**
   * List files in a directory.
   */
  public ls = jest.fn(async (_path: string) => {
    const prefix = `${_path}/`;
   ...
   */
  public mkdir = jest.fn(async (_path: string) => {
    this.filesystem.set(_path, fakeDirectoryData);
    return _path;
  });
...
const mockFileSystem = new FileSystemMock();

jest.mock('react-native-file-access', () => ({
  Dirs,
  FileSystem: mockFileSystem,
  Util: jest.requireActual('react-native-file-access').Util,
}));
